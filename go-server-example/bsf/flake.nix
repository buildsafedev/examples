
{
	  ...
	  devShells = forEachSupportedSystem ({ pkgs, 
		...
		devShell = pkgs.mkShell {
		  ...
		  packages =  [
			...
			nixpkgs-2af19cfb6aa40768c4bbefd801a136270e099191-pkgs.go_1_21  
	...
	ociImages = forEachSupportedSystem ({ pkgs, nix2containerPkgs, system , ...}: {
		...
		ociImage_go-dev_app = nix2containerPkgs.nix2container.buildImage {
		...
		ociImage_go-dev_app_with_dev = nix2containerPkgs.nix2container.buildImage {
			name = "docker.io/holiodin01/go-base-dev";
			copyToRoot = [ inputs.self.packages.${system}.default ];
			config = {
				cmd = [  ];
		...
		ociImage_go-runtime_app = nix2containerPkgs.nix2container.buildImage {
		...
		ociImage_go-runtime_app_with_dev = nix2containerPkgs.nix2container.buildImage {
			name = "docker.io/holiodin01/go-base-runtime";
			copyToRoot = [ inputs.self.packages.${system}.default ];
			config = {
				cmd = [  ];
		...
		ociImage_go-dev_app-as-dir = pkgs.runCommand "image-as-dir" { } "${inputs.self.ociImages.${system}.ociImage_go-dev_app.copyTo}/bin/copy-to dir:$out";
		ociImage_go-dev_app_with_dev-as-dir = pkgs.runCommand "image-as-dir" { } "${inputs.self.ociImages.${system}.ociImage_go-dev_app_with_dev.copyTo}/bin/copy-to dir:$out";
		
		
		
		
		ociImage_go-runtime_app-as-dir = pkgs.runCommand "image-as-dir" { } "${inputs.self.ociImages.${system}.ociImage_go-runtime_app.copyTo}/bin/copy-to dir:$out";
		ociImage_go-runtime_app_with_dev-as-dir = pkgs.runCommand "image-as-dir" { } "${inputs.self.ociImages.${system}.ociImage_go-runtime_app_with_dev.copyTo}/bin/copy-to dir:$out";
